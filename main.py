...
def withdraw(coin, network, address, amount):
    ...
    signature = hmac.new(secret_key.encode('utf-8'), msg=querystring.encode('utf-8'),
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"成功提币 {amount} {coin} 到地址: {address}, ID: {response_data.get('id', '未知')}, 价值: {usd_value} USDT, 网络: {network}")
...
def batch_withdraw(coin, network, wallets_file="wallets.txt"):
    ...
    for idx, wallet in enumerate(wallets):
        amount = round(random.uniform(AMOUNT_MIN, AMOUNT_MAX), 6)  # 随机金额，精确到 8 位
...
def save_to_excel(records, filename):
    try:
        ...
        for record in records:
            sheet.append([record["time"], record["address"], record["coin"], record["amount"], record["value_usdt"],
                          record["network"]])
        ...
        print(f"保存记录到 Excel 文件时发生异常: {str(e)}")
